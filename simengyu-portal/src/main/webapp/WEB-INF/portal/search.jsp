...
var price="${price}";
	...
	function fqBrand(brandId){
		if(price!=''){
			window.location.href="/search?keyword=${keyword}&sort=${sort2}&brandId="+brandId+"&price="+price; 
		}else{
			window.location.href = "/search?keyword=${keyword}&sort=${sort2}&brandId="+brandId;
		}
	}
	...
	function fqPrice(price){
		if(brandId!=''){
			...
			window.location.href = "/search?keyword=${keyword}&sort=${sort2}&price=" + price;
		}
	}

	function openOrclose(key){
		if(key=='品牌' && price!=''){
			window.location.href = "/search?keyword=${keyword}&sort=${sort2}&price="+price;
		}else if(key=='价格'){
			window.location.href = "/search?keyword=${keyword}&sort=${sort2}&brandId="+brandId;
		}else{
			window.location.href = "/search?keyword=${keyword}&sort=${sort2}";
		}
...
<div id="J_crumbsBar" class="crumbs-bar">
	...
	<div class="crumbs-nav">
		<div class="crumbs-nav-main clearfix">
			...
			<div class="crumbs-nav-item">
				<strong class="search-key">${keyword }</strong><!-- "瑜伽服" -->
			</div>
			<c:if test="${fn:length(map) != 0 }">
			<div class="sl-b-selected J_brandSelected">
				<span class="crumbs-arrow">已选条件：</span>
					<c:forEach items="${map }" var="m">
						<a title="依琦莲（yiqilian）"  href="javascript:openOrclose('${m.key }');" class="crumb-select-item">
		...
		<div class="ml-wrap">
			<div id="J_filter" class="filter">
				<div class="f-line top">
					...
					<span class="fp-text"><b>${solrPageProducts.pageNum }</b><em>/</em><i>${solrPageProducts.pages }</i></span>
					<!-- <a class="fp-prev disabled" href="javascript:;">&lt;</a> -->
					<!-- <a class="fp-next" onclick="SEARCH.page(3)" href="javascript:;" title="使用方向键右键也可翻到下一页哦！">&gt;</a></div> -->
					<c:if test="${solrPageProducts.pageNum > 1}">
						<a class="fp-prev" onclick="searchKeyword('${sort2}',${solrPageProducts.pageNum-1})" href="javascript:;" 
					title="使用方向键左键也可翻到上一页哦！">&lt;</a>
					</c:if>
					
					<c:if test="${solrPageProducts.pageNum < solrPageProducts.pages}">
					<a class="fp-next" onclick="searchKeyword('${sort2}',${solrPageProducts.pageNum+1 })" href="javascript:;" 
					title="使用方向键右键也可翻到下一页哦！">&gt;</a>
					</c:if>
					<div class="f-result-sum">共<span id="J_resCount" class="num">${solrPageProducts.total }</span>件商品</div>
					<span class="clr"></span>
				</div>
				<div class="f-line">
					<div class="f-store">
...
<div id="J_goodsList" class="goods-list-v1 gl-type-1 J-goods-list">
	<ul class="gl-warp clearfix" data-tpl="1">
		<c:forEach items="${solrPageProducts.result }" var="product">
